...
namespace imdexlib {
    ...
    struct typelist {
        ...
        template <typename T>
        static constexpr size_t index_of(std::type_identity<T>) noexcept {
            return index_of_impl<T>(0, typelist<Ts...>());
        }

        template <typename T>
        static constexpr bool contains(std::type_identity<T> type) noexcept {
    ...
    namespace detail {
        ...
        struct ts_applier<typelist<Ts...>> {
            ...
            static void apply(Handler&& handler) {
                [[maybe_unused]]
                const int fold[] = { (handler(std::type_identity<Ts>()), 0)..., 0 };
            }
        };
