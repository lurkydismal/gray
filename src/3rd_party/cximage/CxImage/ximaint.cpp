...
void CxImage::OverflowCoordinates(long &x, long &y, OverflowMethod const ofMethod)
{
  ...
  switch (ofMethod) {
    case OM_REPEAT:
      //clip coordinates
      x=std::max(x,0l); x= std::min(x, head.biWidth-1);
      y=std::max(y,0l); y= std::min(y, head.biHeight-1);
      break;
    case OM_WRAP:
      //wrap coordinates
      x = x % head.biWidth;
...
void CxImage::OverflowCoordinates(float &x, float &y, OverflowMethod const ofMethod)
{
  ...
  switch (ofMethod) {
    case OM_REPEAT:
      //clip coordinates
      x=std::max(x,0.f); x=std::min(x, (float)head.biWidth-1);
      y=std::max(y,0.f); y=std::min(y, (float)head.biHeight-1);
      break;
    case OM_WRAP:
      //wrap coordinates
