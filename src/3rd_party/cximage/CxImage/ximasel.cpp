...
bool CxImage::SelectionAddRect(RECT r, BYTE level)
{
    ...
    if (info.rSelectionBox.top <= r2.top) info.rSelectionBox.top = std::max(0L,std::min(head.biHeight,r2.top+1));
    if (info.rSelectionBox.left > r2.left) info.rSelectionBox.left = std::max(0L,std::min(head.biWidth,r2.left));
    if (info.rSelectionBox.right <= r2.right) info.rSelectionBox.right = std::max(0L,std::min(head.biWidth,r2.right+1));
    if (info.rSelectionBox.bottom > r2.bottom) info.rSelectionBox.bottom = std::max(0L,std::min(head.biHeight,r2.bottom));

    long ymin = std::max(0L,std::min(head.biHeight,r2.bottom));
    long ymax = std::max(0L,std::min(head.biHeight,r2.top+1));
    long xmin = std::max(0L,std::min(head.biWidth,r2.left));
    long xmax = std::max(0L,std::min(head.biWidth,r2.right+1));
...
bool CxImage::SelectionAddEllipse(RECT r, BYTE level)
{
    ...
    if (info.rSelectionBox.left > (xcenter - xradius)) info.rSelectionBox.left = std::max(0L,std::min(head.biWidth,(xcenter - xradius)));
    if (info.rSelectionBox.right <= (xcenter + xradius)) info.rSelectionBox.right = std::max(0L,std::min(head.biWidth,(xcenter + xradius + 1)));
    if (info.rSelectionBox.bottom > (ycenter - yradius)) info.rSelectionBox.bottom = std::max(0L,std::min(head.biHeight,(ycenter - yradius)));
    if (info.rSelectionBox.top <= (ycenter + yradius)) info.rSelectionBox.top = std::max(0L,std::min(head.biHeight,(ycenter + yradius + 1)));

    long xmin = std::max(0L,std::min(head.biWidth,xcenter - xradius));
    long xmax = std::max(0L,std::min(head.biWidth,xcenter + xradius + 1));
    long ymin = std::max(0L,std::min(head.biHeight,ycenter - yradius));
    long ymax = std::max(0L,std::min(head.biHeight,ycenter + yradius + 1));

    long y,yo;
    for (y=ymin; y<std::min(ycenter,ymax); y++){
...
bool CxImage::SelectionAddPolygon(POINT *points, long npoints, BYTE level)
{
    ...
    while (i < npoints){
        ...
        if (localbox.top < r2.top) localbox.top = std::max(0L,std::min(head.biHeight-1,r2.top+1));
        if (localbox.left > r2.left) localbox.left = std::max(0L,std::min(head.biWidth-1,r2.left-1));
        if (localbox.right < r2.right) localbox.right = std::max(0L,std::min(head.biWidth-1,r2.right+1));
        if (localbox.bottom > r2.bottom) localbox.bottom = std::max(0L,std::min(head.biHeight-1,r2.bottom-1));
    ...
    if (info.rSelectionBox.top <= localbox.top) info.rSelectionBox.top = std::min(head.biHeight,localbox.top + 1);
    if (info.rSelectionBox.left > localbox.left) info.rSelectionBox.left = std::min(head.biWidth,localbox.left);
    if (info.rSelectionBox.right <= localbox.right) info.rSelectionBox.right = std::min(head.biWidth,localbox.right + 1);
    if (info.rSelectionBox.bottom > localbox.bottom) info.rSelectionBox.bottom = std::min(head.biHeight,localbox.bottom);
