...
void CScriptStackTracker::print_stack    (lua_State *L)
{
    ...
    for (int j=m_current_stack_level - 1, k=0; j>=0; --j, ++k) {
        ...
        if (!l_tDebugInfo.name)
            g_pScriptEngine->script_log(ScriptStorage::eLuaMessageTypeError,"%2d : [%s] %s(%d) : %s",k,l_tDebugInfo.what,l_tDebugInfo.short_src,l_tDebugInfo.currentline,"");
        else
            if (!xr_strcmp(l_tDebugInfo.what,"C"))
                g_pScriptEngine->script_log    (ScriptStorage::eLuaMessageTypeError,"%2d : [C  ] %s",k,l_tDebugInfo.name);
            else
                g_pScriptEngine->script_log    (ScriptStorage::eLuaMessageTypeError,"%2d : [%s] %s(%d) : %s",k,l_tDebugInfo.what,l_tDebugInfo.short_src,l_tDebugInfo.currentline,l_tDebugInfo.name);
    }
    m_current_stack_level    = 0;
}
