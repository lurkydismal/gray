...
public:
    ...
    MxColor& color(unsigned int i) { VERIFY(colors);  return (*colors)(i); }
    MxNormal& normal(unsigned int i) { VERIFY(normals); return (*normals)(i); }
    MxTexCoord& texcoord(unsigned int i) { VERIFY(tcoords); return (*tcoords)(i); }
