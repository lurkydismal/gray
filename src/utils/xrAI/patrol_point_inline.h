...
IC    const u32 &CPatrolPoint::level_vertex_id                    (const CLevelGraph *level_graph, const IGameLevelCrossTable *cross, const CGameGraph *game_graph) const
{
    VERIFY                (m_initialized);
#ifdef DEBUG
...
IC    const GameGraph::_GRAPH_ID &CPatrolPoint::game_vertex_id    (const CLevelGraph *level_graph, const IGameLevelCrossTable *cross, const CGameGraph *game_graph) const
{
    VERIFY                (m_initialized);
#ifdef DEBUG
    verify_vertex_id    (level_graph,cross,game_graph);
