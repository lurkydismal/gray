...
static struct PPM_CONTEXT {
    ...
    WORD NumStats, Dummy;
    DWORD SummFreq;
    struct STATE { BYTE Symbol, Flag; DWORD Freq; PPM_CONTEXT* Successor; } * Stats;
    PPM_CONTEXT* Suffix;
...
NO_LOOP:
    if (pps == ps)                          return pc;
    UpState.Symbol=*(BYTE*) UpBranch;       UpState.Freq=UpState.Flag=0;
...
{
    STATE& rs=oneState();
    if (rs.Symbol == symbol)                (FoundState=&rs)->Freq++;
    else { NumMasked=0;                     FoundState=NULL; }
}
inline void PPM_CONTEXT::encodeSymbol(int symbol)
{
    ...
    for (int i=NumStats;p->Symbol != symbol;p++)
            if (--i < 0) {
                NumMasked=NumStats;         FoundState=NULL;
                return;
            }
    (FoundState=p)->Freq++;                 SummFreq++;
...
static void EncodeFile2(FILE* DecodedFile)
{
    ...
    rewind(DecodedFile);                    MinContext=MaxContext;
    for (int ns=MinContext->NumStats, NumBytes=0; ;ns=MinContext->NumStats) {
        int c = getc(DecodedFile);
        if ( ns )                           MinContext->encodeSymbol(c);
        else                                MinContext->encodeBinSymbol(c);
        while ( !FoundState ) {
            do {
                MinContext->EscFreq++;      MinContext=MinContext->Suffix;
                if ( !MinContext )          goto STOP_ENCODING;
            } while (MinContext->NumStats == NumMasked);
            MinContext->encodeSymbol(c);
        }
        if ( FoundState->Successor )        MinContext=FoundState->Successor;
        else
            while (MinContext->Suffix != NULL) {
                MinContext=MinContext->Suffix;
                if (!MinContext->NumStats)  FoundState=&(MinContext->oneState());
                else
...
inline double GetExtraBits(double f,double e,double f1,double sf1,double sf0,int ns)
{
    ...
    ExtraBits *= MULT;
    if ( ns )                               ExtraBits += COD_ERR*f;
    else                                    ExtraBits -= COD_ERR*e;
    return ExtraBits;
}
...
void ShrinkModel(int MaxSize)
{
    ...
    for (int k=0; ;k++) {
        ...
        s1=SizeOfModel;                     b1=b;
        if ( !k ) { b++;                    continue; }
        if (s1 != s0)                       b=b0+(b1-b0)/(s1-s0)*(MaxSize-s0);
        else                                b=b1+1.0;
        if (b > 2.0*b1)                     b=2.0*b1;
        else if (b < 0.5*b1)                b=0.5*b1;
