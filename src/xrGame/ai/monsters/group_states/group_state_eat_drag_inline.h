...
void CStateGroupDragAbstract::initialize()
{
    ...
    this->object->character_physics_support()->movement()->PHCaptureObject(const_cast<CEntityAlive* >( this->object->EatedCorpse ), &cb);
    ...
    if (capture && !capture->Failed()) {
        ...
        if (m_cover_vertex_id == u32(-1) || this->object->Position().distance_to(m_cover_position) < 2.f || !this->object->Home->at_min_home(m_cover_position)) {
