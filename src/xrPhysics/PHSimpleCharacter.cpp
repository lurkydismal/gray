...
void CPHSimpleCharacter::PhTune(dReal step)
{
    ...
    bool NewJumping = (b_jumping && b_good_graund || (m_elevator_state.ClimbingState() && b_valide_wall_contact));
    if (NewJumping)
        b_jumping = false;
    ...
    if (b_jump)
    {
        b_lose_control = true;
        b_depart_control = true;
        dBodySetLinearVel(m_body, m_jump_accel.x, m_jump_accel.y, m_jump_accel.z);
    ...
    if (is_control)
    {
        ...
        dBodyAddForce(m_body, m_control_force[0], m_control_force[1], m_control_force[2]);
        if (!b_lose_control || b_clamb_jump)
        {
            dBodyAddForce(m_body,
                -sidedir[0] * vProj * (500.f + 200.f * b_clamb_jump) * m_friction_factor,
                -m.mass * (50.f) * (!b_lose_control && !(is_contact || (b_any_contacts))),
...
bool CPHSimpleCharacter::ValidateWalkOnObject()
{
    ...
    //decide to clamb
    if (!m_elevator_state.Active() && b_valide_wall_contact && (m_contact_count > 1) && (m_wall_contact_normal[1] < M_SQRT1_2) && !b_side_contact)
    {
        b_clamb_jump = ((m_wall_contact_position[0] - m_ground_contact_position[0]) * m_control_force[0] +
    ...
    {
        if (
            dFabs((m_wall_contact_position[0] - m_ground_contact_position[0]) + (m_wall_contact_position[2] - m_ground_contact_position[2])) > 0.05f &&
            m_wall_contact_position[1] - m_ground_contact_position[1] > 0.01f
            )
        {
...
void CPHSimpleCharacter::SetAcceleration(Fvector accel)
{
    ...
    if (!dBodyIsEnabled(m_body) && !fsimilar(0.f, accel.magnitude()))
    {
        Enable();
    }
...
void CPHSimpleCharacter::ApplyAcceleration() 
{
    ...
    if(m_elevator_state.Active())
    {
        ...
        if (m_elevator_state.ClimbingState() && m_elevator_state.GetControlDir(*(Fvector*)m_control_force))
        {
...
void CPHSimpleCharacter::InitContact(dContact* c, bool& do_collide, u16 material_idx_1, u16 material_idx_2)
{
    ...
    float soft_param = dumping_rate + normal[1] * (1.f - dumping_rate);
    if (is_control)
    {
        c->surface.mu = 0.f;
...
void CPHSimpleCharacter::TestRestrictorContactCallbackFun(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1, SGameMtl* material_2)
{
    ...
    if (!obj_data || !obj_data->ph_object)
        return;
