...
void CActorTools::RealUpdateProperties()
{
    ...
    if (m_pEditObject) {
        ...
        if (m_pEditObject->IsSkeleton()) 
        {
            ...
            {
                m_pEditObject->FillMotionList(MOTIONS_PREFIX, MotionList, emMotion);
            }

            // Sort motions 
            std::sort(MotionList.begin(), MotionList.end(), [](ListItem* ItemA, ListItem* ItemB)
            {
