...
namespace ImGui
{
    ...
    bool MIsPopupOpen(const char* pName)
    {
        ImGuiID iID = ImGui::GetID(pName);
        return IsPopupOpen(iID, ImGuiPopupFlags_None);
        //ImGuiState& g = *GImGui;
    ...
    bool BeginPiePopup(const char* pName, int iMouseButton)
    {
        if (MIsPopupOpen(pName))
        {
            ...
            ImGui::SetNextWindowPos(ImVec2(-100.f, -100.f), /*ImGuiSetCond_Appearing*/ImGuiCond_Appearing);
