...
int CImageManager::CreateMergedTexture(u32 layer_cnt, SSimpleImageVec& src_images, SSimpleImage& dst_image, int dest_width, int dest_height, Fvector2Vec& dest_offset, Fvector2Vec& dest_scale, boolVec& dest_rotate, U32Vec& dest_remap)
{
    ...
    for (auto s_it = src_images.begin(); s_it!=src_images.end(); s_it++){
        ...
        for (u32 k=0; k<layer_cnt; k++){
            if (!bRotated)     blit    (dest_layers[k].data(),dest_width,dest_height,s_it->layers[k].data(),s_it->w,s_it->h,R.lt.x,R.lt.y);
            else            blit_r  (dest_layers[k].data(),dest_width,dest_height,s_it->layers[k].data(),s_it->w,s_it->h,R.lt.x,R.lt.y);
        }
    }
...
//.        FS.update_path    (t_name,_textures_,**n_it);
    ...
    for (s_it = src_items.begin(); s_it!=src_items.end(); s_it++){
        ...
        dest_rotate.push_back    (bRotated);
        // Perform BLIT
        if (!bRotated)     blit    (dest_pixels.data(),dest_width,dest_height,s_it->layers.back().data(),s_it->w,s_it->h,R.lt.x,R.lt.y);
        else            blit_r  (dest_pixels.data(),dest_width,dest_height,s_it->layers.back().data(),s_it->w,s_it->h,R.lt.x,R.lt.y);
    }
