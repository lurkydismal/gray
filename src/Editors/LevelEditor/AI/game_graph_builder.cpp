...
void CGameGraphBuilder::load_graph_point(CSE_Abstract* entity)
{
    ...
    vertex.tNodeID = level_graph().valid_vertex_position(vertex.tLocalPoint)
                         ? level_graph().vertex_id(vertex.tLocalPoint)
                         : u32(-1);
    ...
    Memory.mem_copy(vertex.tVertexTypes, graph_point->m_tLocations,
...
void CGameGraphBuilder::build_cross_table()
{
    ...
    for (int i = 0, n = level_graph().header().vertex_count(); i < n; i++)
    {
        ...
        tCrossTableCell.fDistance = float(m_distances[tCrossTableCell.tGraphIndex][i]) * level_graph().header().
            cell_size();
    }
}
...
void CGameGraphBuilder::process_tripple(const TRIPPLE& tripple)
{
    ...
    for (; I != E; ++I)
    {
        ...
        {
            VERIFY(
                _min((*I).weight(),graph().edge((*I).vertex_id(),tripple.second.first) ? graph().edge((*I).vertex_id(),
                    tripple.second.first)->weight() : (*I).weight()) <= tripple.first);
            VERIFY(
                _min(edge->weight(),graph().edge(edge->vertex_id(),tripple.second.second) ? graph().edge(edge->vertex_id
                    (),tripple.second.second)->weight() : (*I).weight()) <= tripple.first);
