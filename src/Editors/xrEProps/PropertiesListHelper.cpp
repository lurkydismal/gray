...
{    return           (CanvasValue*)    AppendValue    (items,key,new CanvasValue(val,height),PROP_CANVAS);              }
...
{    
    PropValue*        PV             = AppendValue    (items,key,new ButtonValue(val,flags),PROP_BUTTON);
    ButtonValue*     BV            = dynamic_cast<ButtonValue*>(PV);
    BV->OnBtnClickEvent            = onClick;
    return                         BV;
//    return           (ButtonValue*)    AppendValue    (items,key,new ButtonValue(val,flags),PROP_BUTTON);                
}
S8Value*         CPropHelper::CreateS8        (PropItemVec& items, shared_str key, s8* val, s8 mn, s8 mx, s8 inc)
{    return           (S8Value*)        AppendValue    (items,key,new S8Value(val,mn,mx,inc,0),PROP_NUMERIC);            }
S16Value*         CPropHelper::CreateS16        (PropItemVec& items, shared_str key, s16* val, s16 mn, s16 mx, s16 inc)
{    return           (S16Value*)        AppendValue    (items,key,new S16Value(val,mn,mx,inc,0),PROP_NUMERIC);         }
S32Value*          CPropHelper::CreateS32        (PropItemVec& items, shared_str key, s32* val, s32 mn, s32 mx, s32 inc)
{   return        (S32Value*)        AppendValue    (items,key,new S32Value(val,mn,mx,inc,0),PROP_NUMERIC);         }
U8Value*         CPropHelper::CreateU8        (PropItemVec& items, shared_str key, u8* val, u8 mn, u8 mx, u8 inc)
{   return        (U8Value*)        AppendValue    (items,key,new U8Value(val,mn,mx,inc,0),PROP_NUMERIC);           }
U16Value*         CPropHelper::CreateU16        (PropItemVec& items, shared_str key, u16* val, u16 mn, u16 mx, u16 inc)
{   return        (U16Value*)        AppendValue    (items,key,new U16Value(val,mn,mx,inc,0),PROP_NUMERIC);         }
...
{   return        (RListValue*)    AppendValue    (items,key,new RListValue(val,lst,cnt),PROP_RLIST);               }
...
{   return        (VectorValue*)    AppendValue    (items,key,new VectorValue(val,0.f,1.f,0,0),PROP_VCOLOR);          }
RTextValue*     CPropHelper::CreateRText    (PropItemVec& items, shared_str key, shared_str* val)
{   return        (RTextValue*)    AppendValue    (items,key,new RTextValue(val),PROP_RTEXT);                        }
STextValue*     CPropHelper::CreateSText    (PropItemVec& items, shared_str key, xr_string* val)
{   return        (STextValue*)    AppendValue    (items,key,new STextValue(val),PROP_STEXT);                        }
...
{    return        (FloatValue*)    AppendValue    (items,key,new FloatValue(val,mn,mx,0,0),PROP_TIME);            }
ShortcutValue*    CPropHelper::CreateShortcut    (PropItemVec& items, shared_str key, xr_shortcut* val)
{    return        (ShortcutValue*)AppendValue    (items,key,new ShortcutValue(val),PROP_SHORTCUT);                                }
...
{   return        (CTextValue*)    AppendValue    (items,key,new CTextValue(val,sz),PROP_CTEXT);                        }
CTextValue*     CPropHelper::CreateTexture    (PropItemVec& items, shared_str key, LPSTR val, u32 sz)
{   return        (CTextValue*)    AppendValue    (items,key,new CTextValue(val,sz),PROP_TEXTURE2);                        }
CListValue*     CPropHelper::CreateCList    (PropItemVec& items, shared_str key, LPSTR val, u32 sz, xr_string* lst, u32 cnt)
{   return        (CListValue*)    AppendValue    (items,key,new CListValue(val,sz,lst,cnt),PROP_CLIST);                   }
