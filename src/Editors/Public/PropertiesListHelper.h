...
public:
    ...
    virtual U8Value*                 CreateU8            (PropItemVec& items, shared_str key, u8* val, u8 mn=0, u8 mx=100, u8 inc=1);
    ...
    virtual RToken8Value*             CreateRToken8        (PropItemVec& items, shared_str key, u8* val,  xr_rtoken* token, u32 t_cnt);
    virtual RToken16Value*             CreateRToken16        (PropItemVec& items, shared_str key, u16* val, xr_rtoken* token, u32 t_cnt);
    virtual RToken32Value*             CreateRToken32        (PropItemVec& items, shared_str key, u32* val, xr_rtoken* token, u32 t_cnt);
    virtual RListValue*              CreateRList            (PropItemVec& items, shared_str key, shared_str* val, shared_str* lst, u32 cnt);
    ...
    virtual FloatValue*             CreateAngle            (PropItemVec& items, shared_str key, float* val, float mn=flt_min, float mx=flt_max, float inc=0.01f, int decim=2);
    virtual VectorValue*             CreateAngle3        (PropItemVec& items, shared_str key, Fvector* val, float mn=flt_min, float mx=flt_max, float inc=0.01f, int decim=2);
    virtual RTextValue*             CreateName            (PropItemVec& items, shared_str key, shared_str* val, ListItem* owner);
    virtual RTextValue*             CreateNameCB        (PropItemVec& items, shared_str key, shared_str* val, TOnDrawTextEvent=0, RTextValue::TOnBeforeEditEvent=0, RTextValue::TOnAfterEditEvent=0);

    virtual GameTypeValue*            CreateGameType        (PropItemVec& items, shared_str key, GameTypeChooser* val);

    // obsolette    
    virtual CTextValue*             CreateCText            (PropItemVec& items, shared_str key, LPSTR val, u32 sz);
    virtual CListValue*              CreateCList            (PropItemVec& items, shared_str key, LPSTR val, u32 sz, xr_string* lst, u32 cnt);
